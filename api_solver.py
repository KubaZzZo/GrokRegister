...
class TurnstileAPIServer:
    ...
    async def _startup(self) -> None:
        ...
        try:
            ...
            # Start periodic cleanup for expired solver results.
    ...
    async def _initialize_browser(self) -> None:
        ...
        for _ in range(self.thread_count):
            if self.browser_type in ['chromium', 'chrome', 'msedge']:
                ...
                # Use default values for camoufox and other browser types.
    ...
    async def _optimized_route_handler(self, route):
        """Route handler that blocks non-essential resources."""
    ...
    async def _find_turnstile_elements(self, page, index: int):
        ...
        for selector in selectors:
            try:
                # Count carefully because locator operations can fail.
                try:
                    count = await page.locator(selector).count()
                except Exception:
                    # Skip this selector if count() raises.
                    continue
    ...
    async def _find_and_click_checkbox(self, page, index: int):
        ...
        try:
            # Try several iframe selectors and tolerate locator failures.
            iframe_selectors = [
            ...
            for selector in iframe_selectors:
                try:
                    ...
                    # Count safely for iframe locators as well.
                    try:
            ...
            if iframe_locator:
                try:
                    # Resolve the frame object from the iframe element.
                    ...
                    if frame:
                        ...
                        for selector in checkbox_selectors:
                            try:
                                # Avoid locator.count() here and try a direct click path instead.
                                try:
                                    # Try the direct checkbox click without a separate count check.
                                    ...
                                    # Log the failed attempt in debug mode and continue.
                                    if self.debug:
                        ...
                        # Fall back to clicking the iframe itself if the checkbox click fails.
                        try:
                            if self.debug:
    ...
    async def _try_click_strategies(self, page, index: int):
        ...
        for strategy_name, strategy_func in strategies:
            try:
                ...
                if result is True or result is None:  # None means success for some strategies.
                    if self.debug:
    ...
    async def _safe_click(self, page, selector: str, index: int):
        """Best-effort click helper with quiet failure handling."""
        try:
            # Try a direct click without a separate count() call.
    ...
    async def _solve_turnstile(self, task_id: str, url: str, sitekey: str, action: Optional[str] = None, cdata: Optional[str] = None):
        ...
        try:
            ...
            # Inject the Turnstile widget directly into the target page.
            if self.debug:
            ...
            # Wait for the widget to load and render.
            ...
            for attempt in range(max_attempts):
                try:
                    # Count token elements carefully because locator calls can fail.
                    try:
                    ...
                    if count == 0:
                        ...
                        # If there is only one token element, check it directly.
                        try:
                        ...
                        # If multiple token elements exist, inspect them one by one.
                        if self.debug:
